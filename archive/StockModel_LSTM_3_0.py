...
# TODO: make hybrid model of LSTM and ARIMA model
...
# Plot Folder
Plots = 'D:/StockAnalytics/LSTM3_Results3'
...
comb_scaled = np.concatenate([train_scaled,test_scaled])

# create sequences and then separate again - prevents data gap in prediction due to timestep
y_comb_dates = list()
x_comb, y_comb = [], []
for i in range(TimeStep,len(comb_scaled)):
    x_comb.append(comb_scaled[i-TimeStep:i,:])
    y_comb.append(comb_scaled[i,target_var])
    #date=traindays.iloc[i]['Date'].strftime('%Y-%m-%d')
    date=data.iloc[i]['Day']
    y_comb_dates.append(date)
x_comb, y_comb = np.array(x_comb), np.array(y_comb)
...
# each batch of size designated by the timestep has an associated y actual,
# this is what we will try to predict
...
test_outcome['Day']     = testdays.iloc[TimeStep-1:-1]['Day']
test_outcome[Predict]   = test.iloc[TimeStep-1:-1][Predict]
