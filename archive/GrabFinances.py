...
# model parms
test_split = 0.7 # train/test split
#BatchSize  = 32 # number of samples to update weights
...
for i in range(TimeStep,len(train_scaled)):
    ...
    date=traindays.iloc[i]['Day'].strftime('%Y-%m-%d')
...
for i in range(TimeStep,len(test_scaled)):
    ...
    date=testdays.iloc[i]['Day'].strftime('%Y-%m-%d')
    ...
    for j in range(0,len(TimeSteps)):
        ...
        for k in range(0,len(Epochs)):
            ...
            for m in range(0,len(Nodes)):
                ...
                plt.savefig(Plots + '/' + Stock + '_LSTM_B'+ str(BatchSize) + '_T' + str(TimeStep) + '_E' + str(Epoch) + '_N' + str(Node) + '.svg')
                plt.close()
