...
# Plot Folder
Plots = 'D:/StockAnalytics/Forecast2'
...
test_outcome = pd.DataFrame(columns=['PredSlope','PredTrend','TrueSlope','TrueTrend'])
test_outcome['PredSlope']   = pred_trend[:,0]
test_outcome['PredTrend']   = pred_trend[:,1]
test_outcome['TrueSlope']   = act_trend[:,0]
test_outcome['TrueTrend']   = act_trend[:,1]
...
fname = Plots + '/' + Stock + '_LSTM_B'+ str(BatchSize) + '_T' + str(TimeStep) + '_N' + str(Node) + '_M' + str(trend_match) + '_C' + str(corr)[:-12] + '_MAE' + str(mae)[:-12] + '_MAPE' + str(mape)[:-11] + '.svg'
plt.savefig(fname)
plt.close()
